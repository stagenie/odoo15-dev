...
class ReturnOrder(models.Model):
    ...
    picking_ids = fields.Many2many(
        'stock.picking',
        'return_order_picking_rel',
        'return_order_id',
        'picking_id',
        string='Bons de livraison',
        domain="[('sale_id', 'in', sale_order_ids), ('state', '=', 'done'), ('picking_type_code', '=', 'outgoing')]",
        states={'draft': [('readonly', False)]},
        readonly=True
    )
    ...
    # Entrepots autorises pour l'utilisateur (via equipe commerciale)
    allowed_warehouse_ids = fields.Many2many(
        'stock.warehouse',
        compute='_compute_allowed_warehouse_ids',
        string='Entrepots autorises'
    )

    # Emplacement de retour (filtre par entrepot)
    return_location_id = fields.Many2one(
        'stock.location',
        string='Emplacement de retour',
        required=True,
        tracking=True,
        default=lambda self: self._default_return_location_id(),
    ...
    # Lien vers le picking de retour (entree de stock)
    return_picking_id = fields.Many2one(
        'stock.picking',
        string='Reception retour',
        readonly=True,
        copy=False
    )

    picking_count = fields.Integer(
        string='Nombre de receptions',
    ...
    def _default_warehouse_id(self):
        ...
        user = self.env.user
        user_team = user.sale_team_id

        if user_team:
            # Chercher l'entrepot lie a l'equipe de l'utilisateur
        ...
        # Fallback: premier entrepot de la societe
        return self.env['stock.warehouse'].search([
            ('company_id', '=', self.env.company.id)
        ], limit=1)

    @api.model
    def _default_return_location_id(self):
    ...
    def _compute_origin_mode(self):
        ...
        origin_mode = config_param.get_param(
            'adi_return_management.return_origin_mode', 'none'
        )
        for record in self:
    ...
    def _compute_allowed_warehouse_ids(self):
        """Recupere les entrepots autorises pour l'utilisateur via son equipe commerciale"""
        for record in self:
            user = self.env.user
            # Recuperer l'equipe commerciale de l'utilisateur
            user_team = user.sale_team_id

            if user_team:
                # Entrepots lies a l'equipe de l'utilisateur
                warehouses = self.env['stock.warehouse'].search([
                    ('team_id', '=', user_team.id),
                    ('company_id', '=', self.env.company.id)
                ])
                if warehouses:
                    record.allowed_warehouse_ids = warehouses
                else:
                    # Si pas d'entrepot lie a l'equipe, tous les entrepots de la societe
                    record.allowed_warehouse_ids = self.env['stock.warehouse'].search([
                        ('company_id', '=', self.env.company.id)
                    ])
            else:
                # Si pas d'equipe, tous les entrepots de la societe
                record.allowed_warehouse_ids = self.env['stock.warehouse'].search([
                    ('company_id', '=', self.env.company.id)
                ])
    ...
    def _compute_allowed_location_ids(self):
        ...
        for record in self:
            if record.warehouse_id:
                # Emplacements internes enfants de view_location_id de l'entrepot
    ...
    def _onchange_partner_id(self):
        """Reinitialise les commandes et BL quand le client change"""
    ...
    def _onchange_sale_order_ids(self):
        """Met a jour les BL disponibles et retire ceux non lies"""
        if self.sale_order_ids:
            # Filtrer les pickings pour ne garder que ceux des commandes selectionnees
            valid_pickings = self.picking_ids.filtered(
                lambda p: p.sale_id in self.sale_order_ids
            )
    ...
    def _onchange_warehouse_id(self):
        ...
        # Definir l'emplacement par defaut (stock principal de l'entrepot)
        if self.warehouse_id:
            self.return_location_id = self.warehouse_id.lot_stock_id
    ...
    def action_validate(self):
        """Valide l'ordre de retour et cree l'entree de stock"""
        for order in self:
            ...
            if order.origin_mode == 'strict' and not order.picking_ids:
                raise UserError(_(
                    ...
                    "Veuillez selectionner au moins un bon de livraison."
                ))

            # Creer le picking de retour (entree de stock)
            picking = order._create_return_picking()
            order.return_picking_id = picking.id

            order.state = 'validated'
            order.message_post(body=_("Ordre de retour valide par %s. Reception %s creee.") % (
                self.env.user.name, picking.name
            ))

    def _create_return_picking(self):
        """Cree le picking de retour (reception depuis client)"""
        self.ensure_one()

        # Recuperer le type d'operation de reception de l'entrepot
        picking_type = self.warehouse_id.in_type_id

        # Emplacement source: client (partner_id)
        # On utilise l'emplacement client par defaut
    ...
    def action_create_refund(self):
        ...
        invoice_lines = []
        for line in self.line_ids:
            # Recuperer les taxes du produit
        ...
        refund_vals = {
            ...
            'ref': _("Retour %s - %s") % (self.name, self.reason_id.name),
        ...
        self.message_post(body=_("Avoir %s cree") % refund.name)
    ...
    def action_draft(self):
        ...
        for order in self:
            ...
            if order.return_picking_id:
                raise UserError(_(
                    "Impossible de remettre en brouillon un ordre avec reception de stock."
                ))
            order.state = 'draft'
