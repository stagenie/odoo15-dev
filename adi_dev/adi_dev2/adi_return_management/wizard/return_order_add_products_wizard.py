...
class ReturnOrderAddProductsWizard(models.TransientModel):
    _name = 'return.order.add.products.wizard'
    _description = 'Assistant ajout produits retour'
    ...
    line_ids = fields.One2many(
        'return.order.add.products.line',
        'wizard_id',
    ...
    @api.depends('search_filter', 'line_ids')
    ...
    def default_get(self, fields_list):
        ...
        return_order = self.env['return.order'].browse(return_order_id)
    ...
    def _get_available_products(self, return_order):
        ...
        - qty_delivered: Quantite livree
        - price_unit: Prix de vente
        - picking_id: ID du BL (optionnel)
        - sale_line_id: ID de la ligne de vente (optionnel)
        """
        products = []
        ...
        if origin_mode == 'strict' and return_order.picking_ids:
            # Mode strict: produits des BL selectionnes
            for picking in return_order.picking_ids:
                for move_line in picking.move_line_ids:
                    product = move_line.product_id
                    # Chercher le prix et la remise dans la commande de vente
                    ...
                    if existing:
                        existing['qty_delivered'] += move_line.qty_done
                    else:
                        products.append({
                            'product_id': product.id,
                            'qty_delivered': move_line.qty_done,
                            'price_unit': price,
                            'discount': discount,
            ...
            # Mode souple: tous les produits livres au client
    ...
    def action_add_products(self):
        ...
        check_qty = self.env['ir.config_parameter'].sudo().get_param(
            'adi_return_management.return_check_qty_exceeded', 'True'
        )
        if check_qty in ('True', 'true', '1', True):
            for line in selected_lines:
                if line.qty_delivered > 0 and line.qty_to_return > line.qty_delivered:
                    raise UserError(_(
                        "La quantite a retourner (%.2f) pour '%s' depasse la quantite livree (%.2f)."
                    ) % (line.qty_to_return, line.product_id.display_name, line.qty_delivered))
        ...
        for line in selected_lines:
            ...
            # Ajouter le lien vers la ligne de vente si disponible
            if line.sale_line_id:
                vals['sale_line_id'] = line.sale_line_id.id

            # Ajouter le lien vers le BL d'origine si disponible
            if line.picking_id:
                vals['origin_picking_id'] = line.picking_id.id

            self.env['return.order.line'].create(vals)

        return {'type': 'ir.actions.act_window_close'}

    def action_select_all(self):
        """Selectionne tous les produits (filtrés si recherche active)"""
    ...
    def action_deselect_all(self):
        """Deselectionne tous les produits (filtrés si recherche active)"""
...
class ReturnOrderAddProductsLine(models.TransientModel):
    _name = 'return.order.add.products.line'
    _description = 'Ligne assistant ajout produits'
    _order = 'match_filter desc, id'

    wizard_id = fields.Many2one(
        'return.order.add.products.wizard',
        string='Wizard',
        required=True,
        ondelete='cascade'
    )
    ...
    qty_delivered = fields.Float(
        string='Qte livree',
        help="Quantite totale livree au client"
    )
