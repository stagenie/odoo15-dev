...
class InitialCashCountWizard(models.TransientModel):
    _name = 'initial.cash.count.wizard'
    _description = 'Wizard de Comptage Initial de Caisse'
    ...
    balance_start = fields.Monetary(
        string='Solde de départ théorique',
        currency_field='currency_id',
        related='closing_id.balance_start',
        readonly=True
    )
    ...
    @api.depends('total_counted', 'balance_start')
    def _compute_difference(self):
        """Calculer l'écart entre le total compté et le solde de départ théorique"""
        for wizard in self:
            wizard.difference = wizard.total_counted - wizard.balance_start
    ...
    def _prepare_wizard_lines(self):
        ...
        # Vérifier s'il y a déjà des lignes de comptage initial dans la clôture
        existing_counts = {
            line.denomination_id.id: line.quantity
            for line in closing.initial_count_line_ids
        }
    ...
    def action_confirm(self):
        """Valider le comptage initial et mettre à jour la clôture"""
        self.ensure_one()
        ...
        # La validation du comptage obligatoire se fait via le flag initial_count_done

        # Supprimer les anciennes lignes de comptage initial
        closing.initial_count_line_ids.unlink()

        # Créer les nouvelles lignes de comptage initial
        count_lines = []
        ...
        closing.write({
            'initial_count_line_ids': count_lines,
            'initial_count_done': True,  # Marquer le comptage comme effectué (même si total = 0)
        })

        # Message dans le chatter
        closing.message_post(
            body=_(
                "Comptage initial effectué via le wizard<br/>"
                "Total billets : %s %s<br/>"
                "Total pièces : %s %s<br/>"
                "<strong>Total initial compté : %s %s</strong><br/>"
                ...
                self.balance_start, self.currency_id.symbol,
...
class InitialCashCountWizardLine(models.TransientModel):
    _name = 'initial.cash.count.wizard.line'
    _description = 'Ligne de Comptage Initial Wizard'
    _order = 'denomination_type asc, denomination_value desc'  # Billets d'abord, puis valeur décroissante

    wizard_id = fields.Many2one(
        'initial.cash.count.wizard',
        string='Wizard',
        required=True,
        ondelete='cascade'
    )
